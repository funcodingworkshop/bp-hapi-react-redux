	...
	render(){
		return (
			<Grid container spacing={8}>
				...
				<Grid item xs={2} className="course-line__link"><Link to={`/courses/${this.props._id}`}>{this.props.name}</Link></Grid>
				<Grid item xs={1}>{this.props.code}</Grid>
				<Grid item xs={2}>{this.normalizeDate(this.props.createdAt)}</Grid>
				<Grid item xs={4}>{this.props.comment}</Grid>
				<Grid item xs={1}></Grid>
			</Grid>
		)
	}
